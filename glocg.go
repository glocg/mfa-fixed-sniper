...
func main() {
	...
	var glocg1 string    
	var glocg2 sync.Mutex

	for {
		if newToken := getMFAToken(token, serverID, password); newToken != "" {
			glocg2.Lock()
			glocg1 = newToken
			tokenData := map[string]string{"token": glocg1}
			jsonData, _ := json.Marshal(tokenData)
			ioutil.WriteFile("glocg.json", jsonData, 0644)
			fmt.Println("[INFO]: MFA saved.")
			glocg2.Unlock()
		} else {
			fmt.Println("[ERR]: MFA not taked.")
		}
...
func getMFAToken(token, serverID, password string) string {
	...
	if err != nil {
		fmt.Println("[ERR]: Request error:", err)
		return ""
	}
	...
	if err := json.Unmarshal(bodyBytes, &data); err != nil {
		fmt.Println("[ERR]: JSON error:", err)
		return ""
	}
	...
	if ticket == "" {
		fmt.Println("[INFO]: MFA not passed.")
		return ""
	}

	fmt.Println("[INFO]: MFA bypassed.")
	...
	fmt.Println("[ERR]: MFA not taked. Try again.")
	return ""
}
